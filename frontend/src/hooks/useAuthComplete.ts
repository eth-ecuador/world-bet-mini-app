...
import { signOutComplete } from '@/auth/signout';
...
export function useAuthComplete() {
  ...
  const [externalApiAuthenticated, setExternalApiAuthenticated] = useState(authService.isAuthenticated());
  ...
  useEffect(() => {
    ...
    // If there's no session but we're still logged in to the external API
      ...
      authService.logout()
        ...
        .catch(error => console.error('Error logging out from external API:', error));
    }
