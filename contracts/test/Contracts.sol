...
contract ContractsTest is Test {
    ...
    function setUp() public {
        ...
        // Fund rewarder - fix by directly transferring tokens
        ggtToken.approve(address(rewarder), 5000 ether);
        rewarder.fund(5000 ether);

        // Double-check rewarder has received tokens
        console.log(
            "Rewarder initial balance:",
            ggtToken.balanceOf(address(rewarder))
        );
    ...
    function testUnstakeToken() public {
        ...
        assertEq(
            pool.stakeBalances(alice, address(ggtToken)),
    ...
    function testClaimRewards() public {
        ...
        // Check rewarder's token balance before claim
        uint256 rewarderBalance = ggtToken.balanceOf(address(rewarder));
        console.log("Rewarder balance before claim:", rewarderBalance);

        // Verify rewards calculation
        vm.startPrank(alice);
        uint256 balanceBefore = ggtToken.balanceOf(alice);
        console.log("Alice balance before claim:", balanceBefore);

        // Calculate expected rewards manually
        uint256 rewardDuration = 30 days;
        uint256 rewardRate = 1e16; // 0.01 tokens per second
        uint256 expectedReward = rewardDuration * rewardRate;
        ...
        console.log("Alice balance after claim:", balanceAfter);
        uint256 actualReward = balanceAfter - balanceBefore;
        console.log("Actual reward received:", actualReward);
